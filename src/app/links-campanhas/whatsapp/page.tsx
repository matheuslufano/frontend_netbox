...
import WhatsAppLinksTable, { WhatsAppLinkDraft } from "./WhatsAppLinksTable";
...
export default function WhatsAppLinkPage() {
  ...
  const selectedAffiliate = activeAffiliates.find(
    (item) => item.id === Number(affiliateId),
  );
  ...
  const loadItems = useCallback(
    async () => setItems(await listarLinksWhatsApp()),
    [],
  );
    ...
    Promise.all([
      ...
      .then(([affiliateData, linkData, config]) => {
        ...
        if (config.whatsappNumber)
          setPhone(maskBrazilianPhone(config.whatsappNumber));
      })
      .catch((err) =>
        setError(
          getApiErrorMessage(err, "Não foi possível carregar o gerador."),
        ),
      )
  ...
  function validate() {
    ...
    if (linkName.trim().length > 120)
      return "O nome do link deve ter no máximo 120 caracteres.";
  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        ...(codeMode === "existing"
          ? { affiliateCodeId: Number(affiliateCodeId) }
          : { generateNewCode: true }),
        whatsappNumber: normalizeBrazilianPhone(phone),
        message,
        appendAffiliateCode: appendCode,
        identificationTemplate: template,
      };
      ...
      setNotice(
        editingId ? "Link atualizado com sucesso." : "Link salvo com sucesso.",
      );
      await loadItems();
    } catch (err) {
      setError(getApiErrorMessage(err, "Não foi possível salvar o link."));
    } finally {
      setSaving(false);
    }
  }
  ...
  function tablePayload(item: WhatsAppLinkItem, draft: WhatsAppLinkDraft) {
    ...
    return {
      ...
      appendAffiliateCode: draft.appendAffiliateCode,
      identificationTemplate: draft.identificationTemplate,
    };
  }

  async function saveTableEdit(item: WhatsAppLinkItem, draft: WhatsAppLinkDraft) {
    try {
      await editarLinkWhatsApp(item.id, tablePayload(item, draft));
      await loadItems();
      setError("");
      setNotice("Link atualizado com sucesso.");
    } catch (err) {
      throw new Error(getApiErrorMessage(err, "Não foi possível editar o link."));
    }
  }
  ...
  async function saveTableDuplicate(item: WhatsAppLinkItem, draft: WhatsAppLinkDraft) {
    try {
      ...
      throw new Error(getApiErrorMessage(err, "Não foi possível duplicar o link."));
    }
  }

  async function deleteTableItem(item: WhatsAppLinkItem) {
    try {
      await apagarLinkWhatsApp(item.id);
      await loadItems();
      setError("");
      setNotice("Link apagado com sucesso.");
    } catch (err) {
      throw new Error(getApiErrorMessage(err, "Não foi possível apagar o link."));
    }
  }
  ...
  function downloadQr() {
    ...
    anchor.href = savedResult.qrCode;
    anchor.download = `whatsapp-link-${savedResult.id}.png`;
    anchor.click();
  }
  ...
  if (loading)
    return (
      <main className={styles.page}>
        <p className={styles.loading}>Carregando gerador...</p>
      </main>
    );
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        <Link href="/links-campanhas" className={styles.back}>
          <FiArrowLeft /> Links e Campanhas
        </Link>
        <div className={styles.heroIcon}>
          <FaWhatsapp />
        </div>
        <div>
          <span className={styles.kicker}>Rastreamento e conversão</span>
          <h1>Links personalizados do WhatsApp</h1>
          <p>
            Vincule individualmente cada afiliado e seu código a uma mensagem
            rastreável.
          </p>
        </div>
      </header>
      ...
      <div className={styles.builderGrid}>
        <section className={styles.formCard}>
          <div className={styles.sectionTitle}>
            <FiLink />
            <div>
              <h2>Configuração do link</h2>
              <p>Defina a atribuição antes de montar a mensagem.</p>
            </div>
          </div>
          <label className={styles.field}>
            ...
            <input
              ...
              maxLength={120}
              placeholder="Ex.: Campanha Setembro — João"
              autoComplete="off"
            />
          </label>
          <label className={styles.field}>
          ...
          <fieldset className={styles.codeBox}>
            ...
            <div className={styles.segmented}>
              ...
              <label>
                <input
                  type="radio"
                  checked={codeMode === "new"}
                  onChange={() => setCodeMode("new")}
                />{" "}
                Gerar novo código
              </label>
            </div>
            {codeMode === "existing" ? (
              <label className={styles.field}>
                <span>Código / link existente</span>
                <select
                  value={affiliateCodeId}
                  onChange={(e) => setAffiliateCodeId(e.target.value)}
                  disabled={!affiliateId}
                >
                  <option value="">
                    {codes.length
                      ? "Selecione"
                      : "Nenhum código para este afiliado"}
                  </option>
                  {codes.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.code} •{" "}
              ...
              <p className={styles.info}>
                Um novo código de 8 caracteres será criado e vinculado
                individualmente ao afiliado selecionado.
              </p>
            )}
          </fieldset>
          <label className={styles.field}>
          ...
          <label className={styles.switchRow}>
            ...
            />
            <span>
              <strong>Adicionar identificação do afiliado à mensagem</strong>
              <small>Recomendado para atribuição futura no Chatmix.</small>
            </span>
          </label>
          {appendCode && (
            <label className={styles.field}>
        ...
        <aside className={styles.previewColumn}>
          <WhatsAppPreview message={finalMessage} />
          {generatedUrl && (
            <section className={styles.resultCard}>
              <span className={styles.kicker}>Resultado</span>
              <h2>Link gerado</h2>
              <input readOnly value={generatedUrl} aria-label="Link gerado" />
              <div className={styles.resultActions}>
                <button type="button" onClick={() => copyUrl(generatedUrl)}>
                  <FiCopy /> Copiar link
                </button>
                <a href={generatedUrl} target="_blank" rel="noreferrer">
                  <FiExternalLink /> Abrir WhatsApp
                </a>
                {savedResult && (
              ...
              {savedResult && (
                <div className={styles.qr}>
                  <img
                    src={savedResult.qrCode}
                    alt="QR Code do link WhatsApp"
                  />
                  <small>
                    ID do link: {savedResult.id} • Código:{" "}
                    {savedResult.affiliateCode}
                  </small>
                </div>
              )}
            </section>
